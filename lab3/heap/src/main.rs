use crate::heap::{
    linear_heap, nlogn_heap, print_in_order, print_level_order, print_post_order, print_pre_order,
};
use std::time::Instant;
mod file_manipulations;
use chrono::Utc;
enum Algorithm {
    linear,
    nlogn,
}

mod heap;

fn main() {
    let test_data = vec![10, 12, 1, 14, 6, 5, 8, 15, 3, 9, 7, 4, 11, 13, 2];

    print!("Initial:  ");
    print_level_order(&test_data);
    println!();
    let mut linear_heap_vec = linear_heap(&test_data);
    print!("Result:   ");
    print_level_order(&linear_heap_vec);
    println!("");
    println!("Expected: 1, 3, 2, 12, 6, 4, 8, 15, 14, 9, 7, 5, 11, 13, 10");

    let heap_vec = nlogn_heap(&test_data);
    println!("Time for other one!");
    print!("Result:   ");
    print_level_order(&heap_vec);
    println!("");
    println!("Expected: 1, 3, 2, 6, 7, 5, 4, 15, 14, 12, 9, 10, 11, 13, 8");
    println!("Printing different orders: ");
    print!("Preorder:  ");
    print_pre_order(&test_data, 0);

    println!("");
    print!("Inorder:   ");
    print_in_order(&test_data, 0);

    println!("");
    print!("Postorder: ");
    print_post_order(&test_data, 0);
    println!("");
    print!("Levelorder: ");
    print_level_order(&test_data);
    
    println!("");
    let first_element = linear_heap_vec.remove(0); 
println!("Removed: {}", first_element); // 10
println!("New Vec: {:?}", linear_heap_vec);

    println!("");
    println!(":(");
    println!("Doing lame calculations..");
    let unsorted_vec: Vec<u8> = file_manipulations::get_text_num();

    let mut output = String::new();
    output = output + &Utc::now().to_string() + "\n";
    output.push_str(&" ".repeat(16));

    let length = [1000,10000,100000,250000,500000, 750000, 1000000];

    let header = length
        .iter() // Create an iterator over the array
        .map(|&duration| format!("{:>12.2}", duration as u32))
        .collect::<Vec<String>>() // Collect into a Vec<String>
        .join("");

    output.push_str(&header);

    let algos = [Algorithm::linear, Algorithm::nlogn];

    for algo in algos {
        let name = match algo {
            Algorithm::linear => "\nLinear Heap     ",
            Algorithm::nlogn => "\nNlogn Heap      ",
        };
        output.push_str(name);
        for len in length {
            let start = Instant::now();

            let _ = match algo {
                Algorithm::linear => linear_heap(&mut unsorted_vec[0..len].to_vec()),
                Algorithm::nlogn => nlogn_heap(&mut unsorted_vec[0..len].to_vec()),
            };
            let duration = start.elapsed();
            output.push_str(&format!("{:>12.4}", &duration.as_nanos()));
        }
    }

    let _ = file_manipulations::write_to_file(&output);
    let _ = file_manipulations::write_to_file("\n");

    //println!("Tests are done! Check out data in src/output.txt");
    println!("Tests are done!");
    println!("{}", output);
}
